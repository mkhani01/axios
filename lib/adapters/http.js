...
module.exports = function httpAdapter(config) {
  return new Promise(function dispatchHttpRequest(resolvePromise, rejectPromise) {
    ...
    req.on('error', function handleRequestError(err) {
      ...
      // new: include Node.js TLS/SSL error codes and messages
      var msg = err.code
        ? err.code + (err.message ? ' - ' + err.message : '')
        : err.message || 'Network Error';
      var axiosErr = AxiosError.from(err, null, config, req);
      axiosErr.message = msg;               // <-- override message
      axiosErr.code = err.code;             // <-- attach error code
      axiosErr.originalError = err;         // <-- attach original error object
      reject(axiosErr);
    });
